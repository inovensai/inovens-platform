...
NATIVE={
 'read':schema({'path':S,'file_path':S,'offset':{'type':'integer'},'limit':{'type':'integer'}}),
 'write':schema({'path':S,'file_path':S,'content':S},['content']),
 'edit':schema({'path':S,'file_path':S,'oldText':S,'newText':S}),
 'exec':schema({'command':S,'workdir':S,'timeout':{'type':'number'},'yieldMs':{'type':'number'}}),
 'process':schema({'action':S,'sessionId':S,'data':S})
}
def trusted_tools(requested):
    registry={'platform__'+t['name']:{'type':'function','function':{'name':'platform__'+t['name'],'description':t['description'],'parameters':t['inputSchema']}} for t in TOOLS}
    registry.update({n:{'type':'function','function':{'name':n,'parameters':s}} for n,s in NATIVE.items()})
    return [registry[n] for t in requested if (n:=t.get('function',{}).get('name')) in registry]
